...
class ExamRepImp extends ExamRepo{
  ExamRemoteDataSource _examRemoteDataSource;

  ExamRepImp(this._examRemoteDataSource);


  Future<BaseResponse<List<QuestionEntity>>> getQuestionsByExam(String examId)async{

    final BaseResponse<List<QuestionDTO>> response
    =  await _examRemoteDataSource.getQuestionByExam(examId);
...
//-----------------------------------------------
      ...
      case ErrorResponse<List<QuestionDTO>>():
        return ErrorResponse<List<QuestionEntity>>(errMessage: response.errMessage);
